# A PyGame-based software tool to measure user inputs from an Xbox One Controller.
# David Bell - 2021 for MSCI630

# This code is based on the TextPrint class found in PyGame documentation and
# uses the SceneBase class and Centralized Scene Logic model for game programming
# described by Blake O'Hare at nerdparadise.com (a big thanks to Blake for 
...
sequence_list = [
    ...
    ["LSB", "D-Up", "LSB", "D-Right", "LSB", "D-Down", "LSB", "D-Left"],
    ["LT", "LT", "LT", "RT", "RT", "RT"],
    ["RSB", "X", "RSB", "Y", "RSB", "B", "RSB", "A"],
    ["LT", "RSB", "Y", "RT", "LSB", "D-Down"],

    ["LSB", "RSB", "D-Down", "A", "D-Up", "B", "X"],
    ["RB", "RB", "RB", "RB", "RT","LB", "LB", "LB", "LB", "LT"],
    ...
    # ["A", "B", "X", "Y", "LSB", "RSB", "LT", "RT", "RB", "LB", "D-Left", 
    #                                            "D-Up", "D-Right", "D-Down"],
]
...
# Template class by Blake O'Hare
class SceneBase:
    def __init__(self):
        self.next = self
        self.text = TextPrint()
...
# A modified version of the TextPrint class from the PyGame documentation.
# (It now does text-wrapping for long strings)
...
class TextPrint(object):
    def __init__(self, default_colour=DEFAULT_FG_COLOUR,top_margin=10,left_margin=10,
        ...
        # Init by setting the x and y
        self.clear_screen()

    def println(self, screen, textString, colour = None, size=None):
        # Shortcut function to print and newline in a single call.
    ...
    def print(self, screen, textString, colour = None, size=None):
        ...
        for word in textString.split():
            if self.x + font.size(line + word + " ")[0] > screen.get_size()[0]:
                ...
                line = ""
            line += word + " "
        # Render the text that fits as a bitmap
        lineBitmap = font.render(line, True, _colour)
        # Mark the screen for update with text.
        screen.blit(lineBitmap, (self.x, self.y))
        # Advance the cursor's x by the width of the text.
        # Wrapping is handled above if this is EOL.
    ...
    # Needed a cleaner approach to 'newline'
...
# TrialLogger object used to maintain a log state during trials
# then output between scenes (when there will be a timing lull).
...
# Again, this function is mostly from the nerdparadise.com blog.
...
# The rest is code I wrote within the Centralized Scene Logic in mind.
...
class InstructionScene(SceneBase):
    ...
    def Render(self, screen):
        # Here the instructions are given to the participant.
...
class GameScene(SceneBase):
    def __init__(self, sequence_idx=0):
        ...
        # Too much for this right now though - but would be nicer for implementing press-hold
    ...
    def ProcessInput(self, events, pressed_keys):
        ...
        for event in events:
            ...
            # Triggers and Joystick axes        
                ...
                # This ends up feeling pretty 'normal' when compared to how Xbox 
                # implements it.
    ...
    def Render(self, screen):
        ...
        # Update the sequence display with the current target
...
# This is the last screen the participant will see, 
# the game idles here until the window is closed.
class FinishScene(SceneBase):
    ...
    def ProcessInput(self, events, pressed_keys):
        for event in events:
            if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                # Could add some diagnostic output or result metrics here in future.
                pass
    ...
    def Render(self, screen):
        # Thank the participant and provide next steps.
        self.text.clear_screen()
        screen.fill(DEFAULT_BG_COLOUR)
